...
print(1)
ldm_stable = StableDiffusionPipeline.from_pretrained("pretrainedmodel", use_auth_token=MY_TOKEN).to(device)
print(2)
...
origin_path = "data_generated.json"

base_path = "./GeneratedImage/"

replace_path_before = "./GeneratedImage/Replacement/before/"
replace_path_after = "./GeneratedImage/Replacement/after/"
refine_path_before = "./GeneratedImage/Refinement/before/"
refine_path_after = "./GeneratedImage/Refinement/after/"
reweight_path_before = "./GeneratedImage/Reweight/before/"
reweight_path_after = "./GeneratedImage/Reweight/after/"
...
def save_image(images, re_type):
    global cnt0, cnt1, cnt2
    x0 = Image.fromarray(images[0])
    x1 = Image.fromarray(images[1])

    if re_type == 0:
        x0.save(replace_path_before + f"{cnt0}.png")
        x1.save(replace_path_after + f"{cnt0}.png")
        cnt0 += 1
    if re_type == 1:
        x0.save(refine_path_before + f"{cnt1}.png")
        x1.save(refine_path_after + f"{cnt1}.png")
        cnt1 += 1
    if re_type == 2:
        x0.save(reweight_path_before + f"{cnt2}.png")
        x1.save(reweight_path_after + f"{cnt2}.png")
        cnt2 += 1
...
images, x_t = run_and_display(prompts, controller, latent=None, run_baseline=False, generator=g_cpu)
save_image(images, 1)
'''
...
def get_prompts(origin_path):
    ...
    with open(origin_path, 'r', encoding='utf-8') as file:
        data = json.load(file)
        for item in data:
            before_edit = item["图像编辑前的描述"]
            edit_instruct = item["编辑指令"]
            after_edit = item["图像编辑后的描述"]
            message = [before_edit, after_edit, edit_instruct]
            messages.append(message)

    return messages


messages = get_prompts(origin_path)
for data in messages:
    #print(data[0], data[1])
    g_cpu = torch.Generator().manual_seed(8888)
    prompts = [data[0], data[1]]
    controller = AttentionRefine(prompts, NUM_DIFFUSION_STEPS, cross_replace_steps=.8,
                                 self_replace_steps=.4)
    images, x_t = run_and_display(prompts, controller, latent=None, run_baseline=False, generator=g_cpu)
    save_image(images, 1)
